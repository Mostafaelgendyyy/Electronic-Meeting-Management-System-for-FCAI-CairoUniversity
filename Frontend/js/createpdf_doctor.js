...
ul{
  ...
  .topic{margin-top: 25px;width:100% ;height:auto;}`;
    ...
    try {
      ...
      console.log(token)
      console.log(ID); 
      const response1= await fetch(`http://127.0.0.1:8000/api/doctor/upcomingMeeting/${ID}`, {
        method: 'GET',
        headers: {
    ...
    const upcomingMeeting = await response1.json();
    console.log(upcomingMeeting);
    const response = await fetch(`http://127.0.0.1:8000/api/doctor/DoctorPDF/${upcomingMeeting.initiatorid}`, {
      method: 'GET',
      headers: {
        'Authorization': 'Bearer ' + token
      }
